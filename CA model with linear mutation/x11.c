...
/* Button 1 is to pause. The action to Button 2 and 3 can be
   set by the arguments. Note that (x,y) will be the x and y
   coordinates (in pixels) from the top-left of the window
   (0,0). This means that x and y will contain margin. */
void HandleMouseEvent(void (*button2)(int x,int y),void (*button3)(int x,int y))
{
  if (graphics) {
	...
	case Button1:
	  printf("Click button to continue\n");
	  for(;;){
	    XNextEvent(display, &event);
	    if (event.type==ButtonPress){
		...
		button2(event.xbutton.x,event.xbutton.y);
	      }
	      else if(event.xbutton.button==Button3){
		button3(event.xbutton.x,event.xbutton.y);
	      }
	    }
	  }
	  break;
...
int CloseDisplayImmediately()
{
  if (graphics) {
    ...
    /* my addition */
    XFree(visual_list);
    /* the next also frees imagedata */
    XDestroyImage(planeimage);
  }
  return (0);
}
